...
__doc__ = "module for the sounds resources"#information describing the purpose of this module
...
class Sound :
	"""store a sound as resource for world items"""
	def __init__(self,sound_path):
		...
		ext= os.path.splitext(sound_path)[1] # work only if not more than 1 .ext
		...
		if ext=='.wav' :
			...
			sound_file = pyogg.OpusFile(sound_path)
			...
			sound_file = pyogg.VorbisFile(sound_path)
			self.bufer= openal.Buffer(sound_file)
		elif ext=='.flac' :
			sound_file = pyogg.FlacFile(sound_path)
			self.bufer= openal.Buffer(sound_file)
		else :
			raise Error
