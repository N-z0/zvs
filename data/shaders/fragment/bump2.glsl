...
in vec3 eye_vector;
in vec3 tangent_vector;
in vec3 bitangent_vector;
...
void main()
{
	...
	mat3 tbn_matrix = mat3(tangent_vector, bitangent_vector, up_vector) ;
	...
	vec3 tbn_normal_vector = normal_vector;//normalize(tbn_matrix * normal_vector);
	vec3 tbn_point_of_view = eye_vector;//tbn_matrix * eye_vector;
	vec3 tbn_light_direction = light_direction;//normalize(tbn_matrix * light_direction);
	...
	vec3 tex_normal= tbn_matrix * normalize( ColorToVector(texture(normal_texture,texture_coord).rgb) );
	vec3 frag_normal= normalize( mix( tbn_normal_vector, tex_normal, blend_factor) );//blend_factor
